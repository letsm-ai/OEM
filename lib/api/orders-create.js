...
export async function handleOrderCreate(request) {
  ...
  let isGuest = false
  let accountCreated = false // set to true if guest opted to create a real account
  if (!session?.user) {
    ...
    // Optional signup during checkout — if a password is supplied we create
    // a full (non-guest) account so the buyer can log in later and see their
    // orders history. Otherwise we create a light "guest" user.
    const gPassword = typeof guest.password === 'string' ? guest.password : ''
    const wantsAccount = !!(gPassword && gPassword.length >= 6)
    ...
    if (gPassword && gPassword.length > 0 && gPassword.length < 6) {
      return (
        NextResponse.json(
    ...
    if (existingUser) {
      ...
      // Upgrade guest → full account if they now provided a password
      if (wantsAccount && (!existingUser.password || existingUser.password === '')) {
        const bcrypt = await import('bcryptjs')
        existingUser.password = await bcrypt.default.hash(gPassword, 10)
        existingUser.isGuest = false
        accountCreated = true
      }
      await existingUser.save()
    } else {
      // Create a new user. If password supplied → full account; else → guest.
      let hashedPassword = ''
      if (wantsAccount) {
        const bcrypt = await import('bcryptjs')
        hashedPassword = await bcrypt.default.hash(gPassword, 10)
        accountCreated = true
      }
