...
import { getProductLimit } from '@/lib/membership'
...
export async function handleProductCreate(request) {
  ...
  // ---- Per-tier product limit (grandfathered) ----
  ...
  if (Number.isFinite(limit)) {
    ...
    if (currentCount >= limit) {
      return err(
        `وصلت للحدّ الأقصى (${limit} منتجات) لباقتك الحالية. رقّ باقتك لإضافة منتجات غير محدودة.`,
        403
      )
    }
  }
...
export async function handleVendorProductsList() {
  ...
  const limitVal = getProductLimit(tier)
  const limit = Number.isFinite(limitVal) ? limitVal : null // null = unlimited
  return json({
    ...
    quota: {
      ...
      remaining: limit === null ? null : Math.max(0, limit - products.length),
      canCreate: limit === null || products.length < limit,
    },
  })
}
