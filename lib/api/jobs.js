...
export async function handleJobApply(request, { params }) {
  ...
  try {
    ...
    try {
      const { sendNewJobApplicationEmail } = await import('@/lib/email')
      const employer = await User.findById(job.postedBy).select('email name').lean()
      if (employer?.email) {
        sendNewJobApplicationEmail({
          to: employer.email,
          name: employer.name,
          ...
          seekerYears: seeker.yearsOfExperience || 0,
        }).catch((e) => console.error('[jobs] new-app email failed:', e?.message))
      }
...
export async function handleEmployerApplicationUpdate(request, { params }) {
  ...
  // ---- Notify the seeker if their status changed to a meaningful outcome ----
  if (
    oldStatus !== app.status &&
    ['SHORTLISTED', 'REJECTED', 'HIRED', 'VIEWED'].includes(app.status)
  ) {
    try {
      const { sendJobStatusChangeEmail } = await import('@/lib/email')
      const seeker = await User.findById(app.seekerUserId).select('email name').lean()
      if (seeker?.email) {
        sendJobStatusChangeEmail({
          to: seeker.email,
          name: seeker.name,
          jobTitle: job.titleAr,
          companyName: job.companyNameAr,
          status: app.status,
          jobId: String(job._id),
        }).catch((e) => console.error('[jobs] status email failed:', e?.message))
      }
    } catch (e) { /* email module optional */ }
  }
  return ok({ success: true, application: serializeApplication(app.toObject()) })
}
...
async function requireAdmin() {
  ...
  if (auth.session.user.role !== 'ADMIN') return { error: 'FORBIDDEN', status: 403 }
  return auth
}

/** GET /api/admin/jobs — list ALL jobs across all companies */
export async function handleAdminJobsList(request) {
  ...
  if (q) {
    const rx = new RegExp(q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i')
    filter.$or = [{ titleAr: rx }, { companyNameAr: rx }]
  }
  const [items, total, totals] = await Promise.all([
    JobPosting.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    JobPosting.countDocuments(filter),
    JobPosting.aggregate([
      { $group: { _id: '$status', c: { $sum: 1 } } },
    ]),
  ])
  const statusCounts = Object.fromEntries(totals.map((x) => [x._id, x.c]))
  return ok({
    total,
    page,
    limit,
...
/** PATCH /api/admin/jobs/:id — admin can toggle featured / status / delete */
export async function handleAdminJobUpdate(request, { params }) {
  const auth = await requireAdmin()
  if (auth.error) return err(auth.error, auth.status)
  const { id } = params || {}
  if (!isValidObjectId(id)) return err('معرّف غير صحيح', 400)
  ...
  if (!job) return err('الوظيفة غير موجودة', 404)
  const body = await request.json().catch(() => ({}))
  if (typeof body.featured === 'boolean') job.featured = body.featured
  if (body.status && ['ACTIVE', 'CLOSED', 'DRAFT'].includes(body.status)) {
    job.status = body.status
  }
  await job.save()
...
/** DELETE /api/admin/jobs/:id — admin force-delete */
export async function handleAdminJobDelete(_request, { params }) {
  const auth = await requireAdmin()
  if (auth.error) return err(auth.error, auth.status)
  const { id } = params || {}
  if (!isValidObjectId(id)) return err('معرّف غير صحيح', 400)
...
/* ================================================================== */
/*                    AI JOB SUGGESTIONS FOR SEEKERS                    */
/* ================================================================== */

/** GET /api/me/job-suggestions — Gemini ranks ACTIVE jobs by fit to seeker profile */
export async function handleJobSuggestions() {
  const auth = await requireSession()
  if (auth.error) return err(auth.error, auth.status)
  await connectDB()
  const seeker = await JobSeeker.findOne({ userId: auth.session.user.id }).lean()
  if (!seeker || !seeker.title) {
    return ok({ items: [], reason: 'PROFILE_INCOMPLETE' })
  }
  const jobs = await JobPosting.find({
    ...
    .limit(50)
    .lean()
  if (jobs.length === 0) return ok({ items: [] })

  // Try Gemini ranking first, fall back to heuristic scoring
  let ranked = null
  try {
    const key = process.env.EMERGENT_LLM_KEY
    if (key) {
      ranked = await rankJobsWithGemini(seeker, jobs, key)
    }
  } catch (e) {
    console.error('[job-suggestions] gemini failed, falling back:', e?.message)
  }
  if (!ranked || ranked.length === 0) {
    ranked = heuristicRank(seeker, jobs).slice(0, 10)
  }
  return ok({ items: ranked.map((j) => serializeJob(j)) })
}

function heuristicRank(seeker, jobs) {
  const skills = new Set((seeker.skills || []).map((s) => s.toLowerCase()))
  const desiredSectors = new Set(seeker.desiredSectors || [])
  const desiredGovs = new Set(seeker.desiredGovernorates || [])
  const desiredWork = new Set(seeker.workModePref || [])
  const desiredEmp = new Set(seeker.employmentTypePref || [])
  return jobs
    .map((j) => {
      let score = 0
      if (desiredSectors.size && desiredSectors.has(j.sector)) score += 4
      if (desiredGovs.size && desiredGovs.has(j.governorate)) score += 3
      if (desiredWork.size && desiredWork.has(j.workMode)) score += 2
      if (desiredEmp.size && desiredEmp.has(j.employmentType)) score += 2
      const jobSkills = new Set((j.skills || []).map((s) => s.toLowerCase()))
      let overlap = 0
      for (const s of skills) if (jobSkills.has(s)) overlap++
      score += overlap * 2
      return { j, score }
    })
...
async function rankJobsWithGemini(seeker, jobs, key) {
  // Compact prompt: ask Gemini to return top-8 ids ordered by fit
  const seekerBrief = {
    title: seeker.title,
    yearsOfExperience: seeker.yearsOfExperience,
    skills: (seeker.skills || []).slice(0, 15),
    desiredSectors: seeker.desiredSectors || [],
    desiredGovernorates: seeker.desiredGovernorates || [],
    workModePref: seeker.workModePref || [],
    employmentTypePref: seeker.employmentTypePref || [],
    bio: (seeker.bio || '').slice(0, 200),
  }
  const jobBrief = jobs.slice(0, 40).map((j) => ({
    id: String(j._id),
    titleAr: j.titleAr,
    sector: j.sector,
    governorate: j.governorate,
  ...
  const prompt = `You are a job-matching assistant for an Omani professional network. Given a job seeker's profile and a list of ${jobBrief.length} active jobs, rank the TOP 8 jobs BEST-suited to the seeker. Return ONLY a JSON array of job ids in ranked order, no other text.

SEEKER:
${JSON.stringify(seekerBrief, null, 2)}

JOBS:
${JSON.stringify(jobBrief)}

Respond with: ["id1","id2",...] (max 8 items).`
  ...
  let ids
  try { ids = JSON.parse(match[0]) } catch (e) { return null }
  if (!Array.isArray(ids)) return null
  // Map ids back to full job docs, preserving order
  const jobMap = new Map(jobs.map((j) => [String(j._id), j]))
  return ids.map((id) => jobMap.get(String(id))).filter(Boolean)
}

/* ================================================================== */
/*                  EMPLOYER SEARCH FOR CANDIDATES                      */
/* ================================================================== */

/** GET /api/employer/seekers?q=&sector=&gov=&workMode=&employmentType= */
export async function handleEmployerSearchSeekers(request) {
  const auth = await requireSession()
  if (auth.error) return err(auth.error, auth.status)
  const ownerCheck = await requireCompanyOwner(auth.session)
  if (ownerCheck.error) return err(ownerCheck.error, ownerCheck.status, { code: ownerCheck.code })
  ...
  const employmentType = searchParams.get('employmentType') || ''
  const page = Math.max(1, parseInt(searchParams.get('page') || '1', 10))
  const limit = Math.min(40, Math.max(6, parseInt(searchParams.get('limit') || '12', 10)))
  const skip = (page - 1) * limit

  const filter = { openToWork: true, profileVisibility: 'PUBLIC', title: { $ne: '' } }
  if (sector) filter.desiredSectors = sector
  if (gov) filter.desiredGovernorates = gov
  if (workMode) filter.workModePref = workMode
  if (employmentType) filter.employmentTypePref = employmentType
  if (q) {
    const rx = new RegExp(q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i')
    filter.$or = [{ fullName: rx }, { title: rx }, { bio: rx }, { skills: rx }]
  }
  const [items, total] = await Promise.all([
    JobSeeker.find(filter).sort({ updatedAt: -1 }).skip(skip).limit(limit).lean(),
    JobSeeker.countDocuments(filter),
  ])
  // Fetch corresponding user emails as fallback (JobSeeker.email may be empty)
  const userIds = items.map((s) => s.userId).filter(Boolean)
  const users = userIds.length
    ? await User.find({ _id: { $in: userIds } }).select('_id email phone').lean()
    : []
  const userMap = new Map(users.map((u) => [String(u._id), u]))

  // Contact info (email/phone) is visible ONLY to authenticated company owners
  // (requireCompanyOwner check above enforces this at the route level).
  return ok({
    total,
    page,
    limit,
