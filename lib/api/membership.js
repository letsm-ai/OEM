 ...
 *   POST /membership/subscribe  — creates a Thawani checkout session (or mock in dev)
 ...
 * Response:
 *   - When Thawani is enabled (production): { requiresPayment: true, redirectUrl, sessionId, membershipId }
 *   - When mocked (dev / no keys):          { success: true, membership, user }
 */
export async function handleMembershipSubscribe(request) {
  ...
  // ---- Mock (dev / no keys) — activate immediately ----
