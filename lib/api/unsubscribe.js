 ...
 *
 * Admin-only:
 *   GET    /api/admin/email-optouts?search=&page=&limit=  → paginated list
 *   GET    /api/admin/email-optouts/export                → CSV download
 *   DELETE /api/admin/email-optouts/:id                   → re-subscribe (remove)
 *
...
/**
 * GET /api/admin/email-optouts?search=&page=&limit=&source=
 * Returns { items, total, page, limit, totalPages }.
 */
export async function handleAdminOptOutList(request) {
  ...
  const q = {}
  if (search) {
    // Regex-safe: escape metacharacters
...
/**
 * GET /api/admin/email-optouts/export → CSV download of ALL opt-outs.
 */
export async function handleAdminOptOutExport() {
  const guard = await requireAdmin()
  if (guard.error) {
    return NextResponse.json(
      { error: guard.error },
  ...
  await connectDB()
  const rows = await EmailOptOut.find({}).sort({ createdAt: -1 }).lean()
  ...
  const header = ['email', 'source', 'reason', 'ip', 'userAgent', 'createdAt']
  const csvLines = [
    header.join(','),
    ...rows.map((r) =>
      [
  ...
  // Add BOM for Excel to detect UTF-8 correctly (Arabic reasons)
  const csv = '\uFEFF' + csvLines.join('\n')
  const filename = `email-optouts-${new Date().toISOString().slice(0, 10)}.csv`
...
/**
 * DELETE /api/admin/email-optouts/:id → re-subscribe (removes the opt-out row).
 */
export async function handleAdminOptOutDelete(id) {
  const guard = await requireAdmin()
  if (guard.error) {
    return NextResponse.json(
      { error: guard.error },
  ...
  await connectDB()
  const res = await EmailOptOut.deleteOne({ _id: id })
