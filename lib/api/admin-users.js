...
export async function handleAdminUsersList(request) {
  ...
  const [total, users, totals, tierAgg] = await Promise.all([
  ...
  const byTier = { FREE: 0, BASIC: 0, GOLD: 0, PLATINUM: 0 }
  for (const row of tierAgg) {
    if (row._id && byTier[row._id] !== undefined) byTier[row._id] = row.count
  }
  return json({
