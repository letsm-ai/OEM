...
export async function handleBroadcastSend(request) {
  ...
  // Send individually so each recipient gets their own List-Unsubscribe token
  // and no one sees other addresses. Small delay avoids hitting Resend rate-limit
  // (2 req/sec on the free plan).
  const isDev = process.env.NODE_ENV !== 'production'
  const delayMs = isDev ? 100 : 550
  for (const [email, name] of recipients) {
    try {
      ...
      if (result?.skipped === 'opted-out') optedOutSkipped++
      else if (result?.error) {
        failCount++
        if (errorSamples.length < 5) {
          errorSamples.push(
            `${email}: ${String(result.error).slice(0, 200)}`
          )
        }
      } else successCount++
    } catch (e) {
      console.error('[broadcast] send failed for', email, e?.message)
      failCount++
      if (errorSamples.length < 5) {
        errorSamples.push(`${email}: ${String(e?.message || e).slice(0, 200)}`)
      }
    }
