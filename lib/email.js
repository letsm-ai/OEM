...
async function sendEmail({ to, subject, html }) {
  ...
  try {
    // Deliverability-friendly one-click unsubscribe (RFC 8058). We only advertise
    // it here to satisfy inbox providers; the target lives on our support page.
    const unsubMailto = REPLY_TO_EMAIL
      ? `<mailto:${REPLY_TO_EMAIL}?subject=unsubscribe>`
      : null
    const unsubUrl = `<${BASE_URL.replace(/\/$/, '')}/unsubscribe>`
