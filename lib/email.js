...
/** Notify the employer when a new application is submitted */
export async function sendNewJobApplicationEmail({ to, name, jobTitle, jobId, seekerName, seekerTitle, seekerYears }) {
  const body = `
    <p style="margin:0 0 12px;color:#374151;font-size:15px;">
      مرحباً ${escapeHtml(name || 'صديقنا')}،
    </p>
    <p style="margin:0 0 16px;color:#374151;font-size:15px;line-height:1.8;">
      لقد تلقّيت طلب تقديم جديد على إعلان الوظيفة
      <strong style="color:#1B3A6B;">"${escapeHtml(jobTitle)}"</strong>.
    </p>
    <div style="margin:16px 0;padding:16px;background:#F8F9FA;border:1px solid #e5e7eb;border-radius:10px;">
      <div style="font-size:12px;color:#6b7280;">المتقدّم</div>
      <div style="font-size:16px;font-weight:700;color:#1B3A6B;margin-top:4px;">${escapeHtml(seekerName)}</div>
      <div style="font-size:13px;color:#374151;margin-top:2px;">${escapeHtml(seekerTitle || '')} — ${Number(seekerYears || 0)} سنة خبرة</div>
    </div>
    ${primaryButton('عرض المتقدّم', `${BASE_URL}/jobs/employer`)}
    <p style="margin:16px 0 0;color:#6b7280;font-size:12px;">
      يمكنك مراجعة ملف المتقدم بالكامل من لوحة إدارة الوظائف.
    </p>
  `
  return sendEmail({
    to,
    subject: `تقديم جديد على وظيفة "${jobTitle}" — مجلس رواد الأعمال`,
    html: baseTemplate({
      preheader: `${seekerName} تقدّم على وظيفة ${jobTitle}`,
      title: 'تقديم جديد',
      body,
    }),
  })
...
/** Notify the seeker when their application status changes */
export async function sendJobStatusChangeEmail({ to, name, jobTitle, companyName, status, jobId }) {
  const STATUS_LABELS = {
    VIEWED: { title: 'تم الاطلاع على تقديمك', color: '#3b82f6', emoji: '👀' },
    SHORTLISTED: { title: 'أخبار سارّة! تم إدراجك في القائمة القصيرة', color: '#f59e0b', emoji: '⭐' },
    HIRED: { title: 'مبروك! تمّ قبولك للوظيفة', color: '#10b981', emoji: '🎉' },
    REJECTED: { title: 'تحديث بخصوص تقديمك', color: '#6b7280', emoji: '📌' },
  }
  const meta = STATUS_LABELS[status] || STATUS_LABELS.VIEWED
  const body = `
    <div style="text-align:center;margin:4px 0 18px;">
      <div style="display:inline-block;width:56px;height:56px;background:${meta.color}20;border:2px solid ${meta.color};border-radius:50%;line-height:54px;font-size:26px;">${meta.emoji}</div>
    </div>
    <h2 style="margin:0 0 10px;font-size:20px;color:#1B3A6B;text-align:center;">${meta.title}</h2>
    <p style="margin:0 0 16px;text-align:center;color:#374151;font-size:15px;line-height:1.8;">
      مرحباً ${escapeHtml(name || 'صديقنا')}، تمّ تحديث حالة تقديمك على وظيفة
      <strong>"${escapeHtml(jobTitle)}"</strong>
      لدى ${escapeHtml(companyName || 'الشركة')}.
    </p>
    ${status === 'HIRED' ? '<p style="text-align:center;color:#10b981;font-weight:700;font-size:16px;">نتمنى لك التوفيق في مسيرتك المهنية الجديدة! 🌟</p>' : ''}
    ${status === 'REJECTED' ? '<p style="text-align:center;color:#6b7280;font-size:13px;">شكراً لاهتمامك. نتمنى لك التوفيق في تقديماتك القادمة.</p>' : ''}
    ${primaryButton('عرض تقديماتي', `${BASE_URL}/jobs/my-applications`)}
  `
  return sendEmail({
    to,
    subject: `${meta.title} — ${jobTitle}`,
    html: baseTemplate({
