...
const VendorApplicationSchema = new Schema(
  {
    ...
    // ---- Vendor onboarding contract (mandatory acceptance) ----
...
const AppointmentSchema = new Schema(
  {
    ...
    discountPercent: { type: Number, default: 0 },
    // ---- Platform commission (5% by default — same policy as store orders) ----
    commissionPercent: { type: Number, default: 5 },
    commissionAmount: { type: Number, default: 0 },
    // Expert's net take-home after platform commission
    expertNetAmount: { type: Number, default: 0 },
    notes: { type: String },
...
/**
 * Admin-composed broadcast/newsletter email campaigns.
 * Tracks each send for audit + history in the admin panel.
 */
const EmailBroadcastSchema = new Schema(
  {
    ...
    // Targeting filters (empty array = ALL of that category)
    tiers: { type: [String], default: [] }, // ['FREE','BASIC','GOLD','PLATINUM']
    roles: { type: [String], default: [] }, // ['MEMBER','VENDOR','EXPERT','ADMIN']
    activeOnly: { type: Boolean, default: true },
    // Send stats
    totalRecipients: { type: Number, default: 0 },
    successCount: { type: Number, default: 0 },
    failCount: { type: Number, default: 0 },
    optedOutSkipped: { type: Number, default: 0 },
    status: {
      type: String,
    ...
    sentBy: { type: String, index: true }, // admin user id
...
/**
 * Admin-authored custom broadcast templates (subject + htmlBody presets).
 * Complements the built-in JS templates in /app/lib/broadcast-templates.js.
 */
const BroadcastTemplateSchema = new Schema(
  {
    ...
    category: {
      type: String,
      enum: ['update', 'offer', 'reminder', 'welcome', 'event', 'other'],
      default: 'other',
      index: true,
    },
    subject: { type: String, required: true, trim: true },
    ...
    createdBy: { type: String, index: true }, // admin user id
    createdByName: { type: String, default: '' },
  },
  { timestamps: true, _id: false }
)
...
// A single job posting created by a company owner.
const JobPostingSchema = new Schema(
  {
    _id: { type: String, default: uuidv4 },
    companyId: { type: String, ref: 'Company', required: true, index: true },
    postedBy: { type: String, ref: 'User', required: true, index: true },
    // Denormalized for faster listing pages
    companyNameAr: { type: String, default: '' },
    companyLogo: { type: String, default: '' },

    titleAr: { type: String, required: true, trim: true },
    titleEn: { type: String, default: '', trim: true },
    descriptionAr: { type: String, required: true }, // rich text
    ...
    salaryCurrency: { type: String, default: 'OMR' },
    ...
    applyDeadline: { type: Date, index: true },
    status: {
      type: String,
...
JobPostingSchema.index({ status: 1, applyDeadline: -1 })
JobPostingSchema.index({ titleAr: 'text', descriptionAr: 'text', skills: 'text' })

// Job-seeker profile — separate from User so we can keep the User doc lean.
const JobSeekerSchema = new Schema(
  {
    ...
    title: { type: String, default: '', trim: true }, // e.g. "مطور واجهات"
    ...
    desiredSectors: { type: [String], default: [] },
    desiredGovernorates: { type: [String], default: [] },
    workModePref: { type: [String], default: [] },
    employmentTypePref: { type: [String], default: [] },
    ...
    educationSummary: { type: String, default: '', maxlength: 1000 },

    // Free-form experience list — objects, not sub-docs
    experience: [
      new Schema(
        {
          ...
          to: { type: String, default: '' }, // "YYYY-MM" or "Present"
          description: { type: String, default: '' },
        },
        { _id: false }
      ),
    ...
    // Custom links (LinkedIn, portfolio, GitHub, personal site, ...)
    links: [
      new Schema(
        {
          label: { type: String, required: true }, // "LinkedIn" | "Portfolio" | custom
    ...
    phone: { type: String, default: '' },
    email: { type: String, default: '' }, // synced from User
...
// A single application from a seeker to a job posting.
const JobApplicationSchema = new Schema(
  {
    _id: { type: String, default: uuidv4 },
    jobId: { type: String, ref: 'JobPosting', required: true, index: true },
    seekerUserId: {
      type: String,
      ref: 'User',
      required: true,
    ...
    // Snapshot at submission time so future profile edits don't retro-alter
    // what the employer originally saw.
    seekerSnapshot: {
      fullName: String,
      title: String,
      bio: String,
    ...
    coverLetter: { type: String, default: '', maxlength: 1500 },
    ...
    employerViewedAt: { type: Date, default: null },
    employerNotes: { type: String, default: '', maxlength: 1000 },
  },
  { timestamps: true, _id: false }
)
// Prevent duplicate applications from the same seeker to the same job
JobApplicationSchema.index({ jobId: 1, seekerUserId: 1 }, { unique: true })

// Constants export for use in APIs
export const JOB_CONSTANTS = Object.freeze({
  EMPLOYMENT_TYPES,
  WORK_MODES,
  EXPERIENCE_LEVELS,
  EDUCATION_LEVELS,
...
export const EmailBroadcast =
  models.EmailBroadcast || model('EmailBroadcast', EmailBroadcastSchema)
export const BroadcastTemplate =
  models.BroadcastTemplate || model('BroadcastTemplate', BroadcastTemplateSchema)
export const JobPosting =
  models.JobPosting || model('JobPosting', JobPostingSchema)
export const JobSeeker =
  models.JobSeeker || model('JobSeeker', JobSeekerSchema)
