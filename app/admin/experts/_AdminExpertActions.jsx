...
export default function AdminExpertActions({ expertId, currentStatus }) {
  ...
  const approve = async () => {
    setLoading(true)
    const res = await fetch(`/api/admin/experts/${expertId}/approve`, {
      method: 'POST',
    })
    setLoading(false)
    if (res.ok) router.refresh()
    else alert('تعذر الاعتماد')
  }

  const reject = async () => {
    if (!reason.trim()) return alert('أدخل سبب الرفض')
    setLoading(true)
  ...
  return (
    <div className="flex w-full flex-col gap-2 md:w-auto md:items-end">
      ...
      {currentStatus !== 'REJECTED' && !showReject && (
        <button
      ...
      {showReject && (
        <div className="w-full min-w-[240px] rounded-lg border border-red-200 bg-red-50 p-2 md:w-64">
          <textarea
            ...
            className="w-full rounded-md border border-red-200 bg-white px-2 py-1 text-xs"
          />
          <div className="mt-2 flex gap-2">
            ...
            >
              {loading ? <Loader2 className="mx-auto h-3.5 w-3.5 animate-spin" /> : 'تأكيد'}
            </button>
            <button
              onClick={() => {
                setShowReject(false)
